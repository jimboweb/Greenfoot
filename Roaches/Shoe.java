...
public class Shoe extends Actor
{
    ...
    public void act() 
    {
        if(!GameOver.isGameOver())
        {
            ...
            //put the if() statement that ends the game (step 8) here
            if(countRoaches()==0)
            {
                GameOver.endGame();
                getWorld().showText("You Won!", 300,200);
            } else if (countRoaches()>100)
            {
                GameOver.endGame();
                getWorld().showText("Sorry, you lost.", 300,200);
            }
        }
    }    

    //Put the new countRoaches() method (step 4) here
