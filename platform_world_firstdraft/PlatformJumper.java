...
public class PlatformJumper extends Actor
{
    ...
    public void move()
    {
        ...
        if(getX()<=sideWidth)           //changed
        {                               //Changed
            deltaX=Math.abs(deltaX);    //Changed
        }                               //Changed
        if(getX()>worldWidth-sideWidth) //Changed
        {                               //Changed
            deltaX=Math.abs(deltaX)*-1; //Changed
        }                               //Changed
    ...
    {
        PlatformWorld myPlatformWorld = (PlatformWorld)getWorld();                //Get the PlatformWorld world and type it to PlatformWorld class so we can call its methods
        if((newY<200&&myPlatformWorld.topBound>0) || (newY>worldHeight-200&&myPlatformWorld.bottomBound<myPlatformWorld.MAPHEIGHT)) 
        {                                           //If we approach the top or bottom of screen
            int shiftY = newY - getY();             //set shiftY to change in Y
            myPlatformWorld.shiftScreen(0, -shiftY);          //shift screen by shiftY
        } else {                                    //if we aren't near top or bottom of screen
            setLocation(getX(),newY);               //set Y location normally
        }                                           
        if((newX<200&&myPlatformWorld.leftBound>0) || (newX>worldWidth-200&&myPlatformWorld.rightBound<myPlatformWorld.MAPWIDTH))
        {                                               //If we approach the right or left side of screen
            ...
            myPlatformWorld.shiftScreen(shiftX, 0);               //shift screen by shiftX
