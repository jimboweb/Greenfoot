...
public class PlatformWorld extends World
{
    ...
    int bottomBound = MAPHEIGHT - getHeight();                          //Where the bottom of the screen is on the map
    int topBound = MAPHEIGHT - getHeight()*2;                 //Where the top of the screen is on the map
    int rightBound = 0 + getWidth();            //Where the right side of the screen is on the map
    OtherPlatformGuy opg = new OtherPlatformGuy();             //create PlatformJumper
    ScrollAroundMe sam = new ScrollAroundMe();
    ...
    {    
        ...
        //Do this first
        //**************************************************************************
        super(600, 400, 1, false); //Changed
        makeMap();  //Calls the function that makes the map from the image
        update();   //Draws the platforms on the screen
        //addObject(sam, getWidth()/2, getHeight()/2);
        addObject(opg, 300, 200); //Add PlatformJumper
    }
