...
public class DriftMove extends Actor
{
    ...
    public void wrapAtEdge()
    {
        ...
        if(x <= leftBound)                                  //If we hit the left bound
            mover.setLocation(rightBound, y);               //go to the right bound

        if(x >= rightBound)                                 //if we hit the right bound
            mover.setLocation(leftBound, y);                //go to the left bound

        if(y <= topBound)                                   //If we hit the top bound
            mover.setLocation(x, bottomBound);              //go to the bottom bound

        if(y >= bottomBound)                                //If we hit the bottom bound
